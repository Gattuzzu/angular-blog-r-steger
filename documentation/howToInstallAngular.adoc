...
Damit über Keycloak authentifiziert werden kann muss das folgende Modul zu Angular hinzugefügt werden:
...
Anschliessend muss das Modul noch installiert werden:
...
Nun muss der Authentifizierungservice noch konfiguriert werden. Siehe dazu:

. Als erstes muss die Konfig wie folgt für diese Projekt erstellt werden: link:./../src/app/core/auth/auth.config.ts[auth.config.ts]
. Anschliessend kann die Konfiguration in link:./../src/app/app.config.ts[app.config.ts] geladen werden.
